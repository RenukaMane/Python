...
int main()
{
    ...
    if(result != cudaSuccess)
    {
        printf("Device Memory allocation failed foe deviceInput2 array.\n");
        cleanup();
        exit(EXIT_FAILURE);
    }
    ...
    if (result != cudaSuccess)
    {
        printf("Host to Device Data Copy is failed for deviceInput2 arrary.\n");
        cleanup();
        exit(EXIT_FAILURE);
    }
    ...
    if (result != cudaSuccess)
    {
        printf("Device to Host Data copy is failed for deviceOutput.\n");
        cleanup();
        exit(EXIT_FAILURE);
    }
    ...
    char str[128];
    if (bAccuracy == false)
       sprintf(str, "Comparison of CPU and GPU Vector Addition is not within accuracy of 0.00001 at array index %d", breakValue);
    else
    ...
    printf("Time taken for Vector Addition on CPU = %.6f milliseconds\n", timeOnCPU);
    printf("Time taken for Vector Addition on GPU = %.6f milliseconds\n",timeOnGPU);
